...
export const exportProducts = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (category_id) {
      where.category_id = BigInt(category_id as string);
    }
 ...
 */
export const exportRevenue = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // Group by date
    const dailyStats: Record<string, { revenue: number; orders: number; discount: number }> = {};
