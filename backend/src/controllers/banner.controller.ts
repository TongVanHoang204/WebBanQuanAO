 ...
 * GET /api/banners
 */
export const getPublicBanners = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const updateBanner = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (images && Array.isArray(images)) {
        ...
        await prisma.banner_images.deleteMany({ where: { banner_id: BigInt(id as string) } });
...
export const reorderBanners = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    await Promise.all(
      orders.map((item: { id: string; sort_order: number }) =>
