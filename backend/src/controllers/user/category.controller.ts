...
export const getDescendantIds = async (parentId: bigint): Promise<bigint[]> => {
  ...
  for (const child of children) {
    const descendantIds = await getDescendantIds(child.id);
...
export const getCategoryProducts = async (
  ...
  try {
    ...
    // Include products from all descendant categories recursively
    const categoryIds = [category.id, ...(await getDescendantIds(category.id))];
