 ...
 */
export const vnpayIpn = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        let vnp_Params = req.query;
        let secureHash = vnp_Params['vnp_SecureHash'];
...
export const getTransactions = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        if (search) {
            where.OR = [
                { transaction_ref: { contains: search as string } },
                { order: { order_code: { contains: search as string } } }
            ];
        }
