...
export class AIService {
    ...
    static CUSTOMER_TOOLS = ['search_products', 'get_top_products', 'get_categories', 'get_low_stock_products', 'get_order_by_id', 'get_my_orders', 'get_my_info', 'search_by_price_range', 'compare_products'];
    // --- SYSTEM PROMPTS ---
...
`;
    ...
    static getCustomerSystemPrompt(profileData) {
        const now = new Date();
        const vietnamTime = now.toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' });
        let styleContext = '';
        if (profileData) {
...
`;
    ...
    // --- CONTENT GENERATION ---
    static async generateContent(prompt, type = 'product_description') {
        try {
            let systemPrompt = '';
...
- Nếu ảnh có nhiều sản phẩm, CHỈ mô tả sản phẩm nổi bật nhất / ở trung tâm`;
        ...
        try {
            ...
            return {
                ...
                search_phrases: Array.isArray(parsed.search_phrases)
                    ? parsed.search_phrases.map((s) => s.toLowerCase().trim()).filter(Boolean)
                    : []
            };
        }
        catch {
    ...
    static scoreProduct(product, analysis) {
        ...
        if (analysis.material) {
            if (nameLC.includes(analysis.material))
                score += 15;
            else if (descLC.includes(analysis.material))
                score += 8;
        }
    ...
    static async getRecentOrders(args) {
        try {
    ...
    static async searchProducts(args) {
        try {
    ...
    static async getLowStockProducts(args) {
        try {
    ...
    static async getOrderById(args) {
        try {
            if (!args.order_id) {
    ...
    static async getCustomerInfo(args) {
        try {
            if (!args.customer_id) {
    ...
    static async getRevenueByPeriod(args) {
        try {
    ...
    static async getTopProducts(args) {
        try {
            const limit = Math.min(Number(args.limit) || 5, 10);
            ...
            const productIds = topProducts.map(p => p.product_id).filter(id => id !== null);
    ...
    static async updateOrderStatus(args, user) {
        try {
            if (!user)
            ...
            if (!order)
                return { error: `Không tìm thấy đơn hàng ${args.order_id}` };
            await this.prisma.$transaction((tx) => transitionOrderStatus(tx, order.id, args.status, user));
            await logActivity({
    ...
    static async updateProductPrice(args, user) {
        try {
            if (!user)
                return { error: 'Bạn cần đăng nhập để thực hiện thao tác này.' };
    ...
    static async createNotification(args, user) {
        try {
            if (!user)
                return { error: 'Bạn cần đăng nhập để thực hiện thao tác này.' };
    ...
    static async getMyOrders(args, user) {
        try {
            if (!user)
                return { message: 'Bạn vui lòng đăng nhập để xem đơn hàng của mình nhé! 🔒' };
            const limit = Math.min(Number(args.limit) || 5, 10);
            const orders = await this.prisma.orders.findMany({
            ...
            if (orders.length === 0)
                return { message: 'Bạn chưa có đơn hàng nào tại ShopFeshen.' };
            return {
                count: orders.length,
                orders: orders.map(o => ({
                    code: o.order_code,
    ...
    static async getMyInfo(user) {
        try {
            if (!user)
                return { message: 'Bạn chưa đăng nhập. Hãy đăng nhập để mình hỗ trợ tốt hơn nhé! 👋' };
    ...
    static async searchByPriceRange(args) {
        try {
    ...
    static async compareProducts(args) {
        try {
            ...
            const getPrice = (p) => Number(p.product_variants[0]?.price || p.base_price);
            const getStock = (p) => p.product_variants.reduce((sum, v) => sum + (v.stock_qty || 0), 0);
            const getSizes = (p) => [...new Set(p.product_variants.map((v) => v.size).filter(Boolean))].join(', ') || 'Freesize';
            // Build comparison
            const comparison = {
                product1: {
                    name: p1.name,
    ...
    // --- MAIN CHAT LOGIC ---
    static async generateChatResponse(messages, user) {
        ...
        const hasCustomSystemPrompt = requestMessages.length > 0 &&
    ...
    // Customer Chat Logic
    static async generateCustomerResponse(history, userMessage, user) {
    ...
    // Shared Logic
    static async runLLMFlow(messages, isCustomer = false, user) {
        let retries = 0;
        const MAX_RETRIES = 1;
        let relatedProducts = [];
            ...
            try {
                ...
                if (toolCall) {
                    if (this.TOOLS.find(t => t.name === toolCall.tool)) {
                        ...
                        const toolResult = await this.executeTool(toolCall.tool, toolCall.args || {}, user);
