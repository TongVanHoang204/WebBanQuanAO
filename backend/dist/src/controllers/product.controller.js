...
const serializeProduct = (product) => {
    return JSON.parse(JSON.stringify(product, (key, value) => typeof value === 'bigint' ? value.toString() : value));
};
export const getProducts = async (req, res, next) => {
    try {
        const { page = '1', limit = '12', search, category, brand, sort = 'newest', min_price, max_price } = req.query;
        ...
        if (min_price || max_price) {
            where.base_price = {};
            if (min_price) {
                where.base_price.gte = parseFloat(min_price);
            }
            if (max_price) {
                where.base_price.lte = parseFloat(max_price);
            }
        }
        // On Sale filter
        ...
        const minDiscountVal = req.query.min_discount ? parseFloat(req.query.min_discount) : 0;
