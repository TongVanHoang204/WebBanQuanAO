...
export const register = async (req, res, next) => {
    try {
        ...
        // Generate JWT token
        const token = jwt.sign({ userId: user.id.toString(), email: user.email, role: user.role }, process.env.JWT_SECRET || 'default-secret', { expiresIn: '7d' });
        // Send welcome email
...
export const toggle2FA = async (req, res, next) => {
    try {
        ...
        await prisma.users.update({
            ...
            data: { two_factor_enabled: Boolean(enabled) }
        });
        res.json({
            success: true,
...
export const updateAddress = async (req, res, next) => {
    try {
        ...
        if (is_default) {
            await prisma.shipping_addresses.updateMany({
                where: { user_id: req.user.id, id: { not: BigInt(id) } },
                data: { is_default: false }
            });
        }
