...
export const createProduct = async (req, res, next) => {
    try {
        ...
        const product = await prisma.$transaction(async (tx) => {
            ...
            const optionMap = new Map(); // "Color" -> OptionID
            const valueMap = new Map(); // "Color:Red" -> ValueID
            if (data.attributes && data.attributes.length > 0) {
                for (const attr of data.attributes) {
                    ...
                    if (!option)
                        throw new Error(`Could not create or find option ${attr.name}`);
                    ...
                    for (const val of attr.values) {
                        ...
                        if (!optionValue)
                            throw new Error(`Could not create value ${val}`);
...
export const getDashboardStats = async (req, res, next) => {
    try {
        const [totalProducts, totalOrders, totalUsers, pendingOrders, recentOrders, topProducts, salesOverTime, ordersThisWeek] = await Promise.all([
...
export const getUsers = async (req, res, next) => {
    try {
        ...
        if (search) {
            where.OR = [
                { full_name: { contains: search } }, // Removed mode: 'insensitive' for compatibility if mysql/prisma config varies, but okay for default
        ...
        orderStats.forEach(stat => {
            if (stat.user_id)
                statsMap.set(stat.user_id.toString(), stat);
        });
...
export const getAnalytics = async (req, res, next) => {
    ...
    `;
        ...
        const productDetails = await prisma.products.findMany({
            where: { id: { in: topProductsRaw.map(p => p.product_id).filter(id => id !== null) } },
